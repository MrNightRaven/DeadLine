					...
					if "linkprotect:on" == pesan.lower():
						if msg._from in admin:
							if msg.to in settingQr:
								...
								cl.sendMessage(msg.to,"「􏿿 Protect Link 」\n Protect Link Already Active in Group {}".format(siska.name))
								ca.sendMessage(msg.to,"「􏿿 Protect Link 」\n Protect Link Already Active in Group {}".format(siska.name))
								cs.sendMessage(msg.to,"「􏿿 Protect Link 」\n Protect Link Already Active in Group {}".format(siska.name))
								cz.sendMessage(msg.to,"「􏿿 Protect Link 」\n Protect Link Already Active")
							else:
								settingQr.append(msg.to)
							siska = cl.getGroup(msg.to)
							cl.sendText(msg.to,"「􏿿 Protect Link 」\n Protect Link Has Been Activated in Group {}".format(siska.name))
