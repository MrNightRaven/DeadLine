...
class LINE:
  ...
  def login(self, mail=None, passwd=None, cert=None, token=None, qr=False, callback=None, www=False):
    ...
    self._headers = {
              'X-Line-Application': 'CHROMEOS\t1.4.17\tDeadLine_Team\t1', 
              'X-Line-Access': self.authToken, 
              'User-Agent': 'DeadLine\6.0'
              }
  ...
  def loginResult(self, callback=None):
    if callback is None:
      ...
      print("DeadLine™")
      print("it's your MID => " + prof.mid)
      print("it's your Name => " + prof.displayName)
      print("it's your AuthToken  => " + self.authToken)
      print("it's your Cert -> " + self.cert if self.cert is not None else "")
